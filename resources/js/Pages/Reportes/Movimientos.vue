...
const seccionesOpciones = [
    { value: 'tarjetas_unidad', label: 'Tarjetas por Unidad' },
    { value: 'tarjetas_mayor', label: 'Tarjetas por Mayor' },
...
const buscar = debounce(() => {
    ...
    router.get(route('reportes.movimientos'), params, { preserveState: true, replace: true });
}, 500);
...
const exportarPdf = () => {
    ...
    window.open(route('reportes.exportar-pdf', 'movimientos') + '?' + params.toString(), '_blank');
};

const exportarExcel = () => {
    const form = document.createElement('form');
    form.method = 'POST';
    form.action = route('reportes.exportar-excel', 'movimientos');
...
const cambiarPagina = (pagina) => {
    ...
    router.get(`${route('reportes.movimientos')}?${params.toString()}`, {}, { preserveState: true });
};

const formatSeccion = (seccionKey) => {
    const op = seccionesOpciones.find(o => o.value === seccionKey);
    return op ? op.label : seccionKey;
};
</script>
...
<template>
    ...
    <PanelLayout>
        <template #titulo-pagina>
            <h2 class="text-xl font-semibold text-gray-800">Reporte de Movimientos Manuales</h2>
        </template>
        ...
        <el-card class="mb-6 shadow-sm border-t-4 border-t-blue-500">
            ...
            <el-form :inline="true" :model="filtros" class="flex flex-wrap gap-4 items-end mb-0">
                ...
                <el-form-item label="Sección Contable" class="mb-0">
                    <el-select v-model="filtros.seccion" placeholder="Todas" clearable style="width: 192px;">
                ...
                <el-table-column label="Monto Bs" width="120" align="right">
                    <template #default="scope">
                        <span class="font-bold" :class="scope.row.monto < 0 ? 'text-red-600' : 'text-blue-600'">
                            {{ Number(scope.row.monto).toFixed(2) }}
                        </span>
                    </template>
                </el-table-column>
                <el-table-column prop="observacion" label="Observación" min-width="200" />
            </el-table>
